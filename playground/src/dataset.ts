...
export function classifySinR(numSamples: number, noise: number):
  Example2D[] {
  let points: Example2D[] = [];
  let radius = 2.5;
  function getCircleLabel(p: Point, center: Point) {
     return (Math.sin(Math.PI * dist(p, center)/(0.5 * radius)) > 0) ? 1 : -1;
  }
  ...
  for (let i = 0; i < numSamples / 2; i++) {
    let r = randUniform(radius, radius * 1.5);
    let angle = randUniform(0, 2 * Math.PI);
    let x = r * Math.sin(angle);
    let y = r * Math.cos(angle);
    let noiseX = randUniform(-radius, radius) * noise;
  ...
  // Generate negative points outside the circle.
  for (let i = 0; i < numSamples / 2; i++) {
    let r = randUniform(radius * 1.7, 2 * radius);
    let angle = randUniform(0, 2 * Math.PI);
    let x = r * Math.sin(angle);
    let y = r * Math.cos(angle);
    let noiseX = randUniform(-radius, radius) * noise;
...
export function classifySinX(numSamples: number, noise: number):
	Example2D[] {
  let points: Example2D[] = [];
  let distance = 2.5;
  let n = numSamples/4;

  function getSinXLabel(p: Point, d: number) {
	let labelSinX:boolean = true;

	if (p.y <= (d * 1.5 + 1)  && p.y >= (d * 1.5 - 1) 
		|| p.y <= (d * -0.5 + 1)  && p.y >= (d * -0.5 - 1)) {
	  labelSinX = false;
	}

	return labelSinX ? 1 : -1;
  }

  for (let k = 0; k < 4; k++) {
    for (let i = 0; i < n; i++) {
	  let x = randUniform(-5, 5);	
	  let xNoise = randUniform(-5, 5) * noise;
  	  let y = Math.sin(x + randUniform(-0.5,0.5)) + distance * (k - 1.5);
	  let yNoise = randUniform(-5, 5) * noise;
	  let label = getSinXLabel({x: x + xNoise, y: y + yNoise}, distance);
