...
export class AppendingLineChart {
    ...
    // Format: "Time \t LossTrain \t LossTest \n"
    getDataString(): string {
        let string_data: string;

        string_data = "#Time \t LossTrain \t LossTest \n";
        //string_data = "test";

        for (let i = 0; i < this.data.length; i++) {
            let x = this.data[i].x;
            let lossTrain = this.data[i].y[0];
            let lossTest = this.data[i].y[1];

            string_data = string_data.concat(`${x} \t ${lossTrain} \t ${lossTest} \n`);
        }
